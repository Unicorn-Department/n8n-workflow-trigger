 ...
 * Description: A simple plugin that allows users to trigger n8n workflows from WordPress using a shortcode.
 * Version: 0.0.2
...
function n8n_base_url_render() {
    ...
    ?>
    <input type='text' name='n8n_trigger_settings[n8n_base_url]' value='<?php echo isset($options['n8n_base_url']) ? esc_attr($options['n8n_base_url']) : ''; ?>' class="regular-text">
    <p class="description"><?php _e('Example: https://your-n8n-instance.com', 'n8n-workflow-trigger'); ?></p>
    <?php
}
...
function n8n_trigger_options_page() {
    ...
    if (isset($_GET['action']) && $_GET['action'] == 'edit' && isset($_GET['workflow_id'])) {
        $edit_mode = true;
        $editing_workflow_id = sanitize_text_field($_GET['workflow_id']);
        $workflows = get_option('n8n_workflow_triggers', array());

        if (isset($workflows[$editing_workflow_id])) {
            $editing_workflow = $workflows[$editing_workflow_id];
        } else {
            $edit_mode = false;
            add_settings_error('n8n_workflow_trigger', 'workflow-not-found', __('Workflow not found.', 'n8n-workflow-trigger'), 'error');
        }
    }
    ...
    <div class="wrap">
        ...
        <h2><?php echo $edit_mode ? __('Edit Workflow Trigger', 'n8n-workflow-trigger') : __('Add New Workflow Trigger', 'n8n-workflow-trigger'); ?></h2>

        <form method="post" action="">
            <?php
            if ($edit_mode) {
                wp_nonce_field('n8n_edit_workflow_action', 'n8n_edit_workflow_nonce');
                echo '<input type="hidden" name="workflow_id" value="' . esc_attr($editing_workflow_id) . '">';
            } else {
                wp_nonce_field('n8n_add_workflow_action', 'n8n_add_workflow_nonce');
            }
            ?>
            <table class="form-table">
                <tr>
                    <th scope="row"><?php _e('Workflow Name', 'n8n-workflow-trigger'); ?></th>
                    <td>
                        <input type="text" name="workflow_name" class="regular-text" required
                               value="<?php echo $edit_mode ? esc_attr($editing_workflow['name']) : ''; ?>">
                    </td>
                </tr>
                <tr>
                    <th scope="row"><?php _e('Webhook ID/Path', 'n8n-workflow-trigger'); ?></th>
                    <td>
                        <input type="text" name="webhook_id" class="regular-text" required
                               value="<?php echo $edit_mode ? esc_attr($editing_workflow['webhook_id']) : ''; ?>">
                        <p class="description"><?php _e('The part of your webhook URL that follows your n8n base URL', 'n8n-workflow-trigger'); ?></p>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><?php _e('Button Text', 'n8n-workflow-trigger'); ?></th>
                    <td>
                        <input type="text" name="button_text" class="regular-text"
                               value="<?php echo $edit_mode ? esc_attr($editing_workflow['button_text']) : 'Trigger Workflow'; ?>">
                    </td>
                </tr>
            </table>
            <?php
            if ($edit_mode) {
                submit_button(__('Update Workflow Trigger', 'n8n-workflow-trigger'));
                echo ' <a href="' . esc_url(admin_url('options-general.php?page=n8n_workflow_trigger')) . '" class="button">' . __('Cancel', 'n8n-workflow-trigger') . '</a>';
            } else {
                submit_button(__('Add Workflow Trigger', 'n8n-workflow-trigger'));
            }
            ?>
        </form>
        ...
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th><?php _e('Workflow Name', 'n8n-workflow-trigger'); ?></th>
                    <th><?php _e('Webhook Path', 'n8n-workflow-trigger'); ?></th>
                ...
                if (!empty($workflows)) {
                    foreach ($workflows as $id => $workflow) {
                        ...
                        <tr>
                            ...
                            <td><?php echo esc_html($workflow['webhook_id']); ?></td>
                            <td><?php echo esc_html($workflow['button_text']); ?></td>
                            <td>
                                <input type="text" readonly value="[n8n_trigger id=&quot;<?php echo esc_attr($id); ?>&quot;]"
                                       class="code" style="width:100%;" onclick="this.select();">
                            </td>
                            <td>
                                <div class="row-actions">
                                    <button type="button" class="button button-small test-workflow"
                                            data-workflow-id="<?php echo esc_attr($id); ?>">
                                        <?php _e('Test', 'n8n-workflow-trigger'); ?>
                                    </button>
                                    <a href="<?php echo esc_url(add_query_arg(array('action' => 'edit', 'workflow_id' => $id), admin_url('options-general.php?page=n8n_workflow_trigger'))); ?>"
                                       class="button button-small">
                                        <?php _e('Edit', 'n8n-workflow-trigger'); ?>
                                    </a>
                                    <form method="post" style="display:inline;">
                                        <?php wp_nonce_field('n8n_delete_workflow_action', 'n8n_delete_workflow_nonce'); ?>
                                        <input type="hidden" name="delete_workflow" value="<?php echo esc_attr($id); ?>">
                                        <button type="submit" class="button button-small">
                                            <?php _e('Delete', 'n8n-workflow-trigger'); ?>
                                        </button>
                                    </form>
                                </div>
                            </td>
                    ...
                    ?>
                    <tr>
                        <td colspan="5"><?php _e('No workflow triggers defined yet.', 'n8n-workflow-trigger'); ?></td>
                    </tr>
                    <?php
                }
                ?>
    ...
    <div id="test-workflow-modal" style="display:none; position:fixed; z-index:9999; left:0; top:0; width:100%; height:100%; background-color:rgba(0,0,0,0.5); overflow:auto;">
        <div style="background-color:#fff; margin:10% auto; padding:20px; border:1px solid #888; width:50%; max-width:500px; border-radius:5px;">
            ...
            <p id="test-workflow-status"><?php _e('Sending request...', 'n8n-workflow-trigger'); ?></p>
            ...
            <div style="text-align:right; margin-top:15px;">
                <button type="button" class="button" id="close-test-modal"><?php _e('Close', 'n8n-workflow-trigger'); ?></button>
            </div>
        </div>
    </div>
    ...
    jQuery(document).ready(function($) {
        ...
        $('.test-workflow').on('click', function() {
            ...
            $('#test-workflow-status').text('<?php _e('Sending request...', 'n8n-workflow-trigger'); ?>');
            ...
            $.ajax({
                ...
                data: {
                    ...
                    nonce: '<?php echo wp_create_nonce('n8n_test_workflow_nonce'); ?>'
                },
                success: function(response) {
                    if (response.success) {
                        $('#test-workflow-status').text('<?php _e('Success! The workflow was triggered.', 'n8n-workflow-trigger'); ?>');
                        $('#test-workflow-response').text(JSON.stringify(response.data, null, 2)).show();
                    } else {
                        $('#test-workflow-status').text('<?php _e('Error triggering workflow', 'n8n-workflow-trigger'); ?>');
                        $('#test-workflow-response').text(JSON.stringify(response.data, null, 2)).show();
                    }
                },
                error: function(xhr, status, error) {
                    $('#test-workflow-status').text('<?php _e('Error: AJAX request failed', 'n8n-workflow-trigger'); ?>');
...
function n8n_process_admin_actions() {
    ...
    if (isset($_POST['n8n_add_workflow_nonce']) && wp_verify_nonce($_POST['n8n_add_workflow_nonce'], 'n8n_add_workflow_action')) {
        if (isset($_POST['workflow_name']) && isset($_POST['webhook_id'])) {
            ...
            wp_redirect(add_query_arg('updated', 'true', admin_url('options-general.php?page=n8n_workflow_trigger')));
            exit;
        }
    }

    // Handle editing a workflow trigger
    if (isset($_POST['n8n_edit_workflow_nonce']) && wp_verify_nonce($_POST['n8n_edit_workflow_nonce'], 'n8n_edit_workflow_action')) {
        if (isset($_POST['workflow_id']) && isset($_POST['workflow_name']) && isset($_POST['webhook_id'])) {
    ...
    if (isset($_POST['n8n_delete_workflow_nonce']) && wp_verify_nonce($_POST['n8n_delete_workflow_nonce'], 'n8n_delete_workflow_action')) {
...
function n8n_test_workflow_ajax_handler() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'n8n_test_workflow_nonce')) {
    ...
    $settings = get_option('n8n_trigger_settings', array());
...
function n8n_trigger_enqueue_scripts() {
    ...
    wp_localize_script('n8n-trigger-script', 'n8nTrigger', array(
...
EOT;
    file_put_contents(N8N_TRIGGER_PLUGIN_DIR . 'assets/js/n8n-trigger.js', $js_content);
...
EOT;
    file_put_contents(N8N_TRIGGER_PLUGIN_DIR . 'assets/css/n8n-trigger.css', $css_content);
}
...
function n8n_trigger_ajax_handler() {
    // Verify nonce
    if (!isset($_POST['nonce']) || !wp_verify_nonce($_POST['nonce'], 'n8n_trigger_nonce')) {
        wp_send_json_error('Invalid nonce');
        return;
    }
...
function n8n_trigger_shortcode($atts) {
    ...
    $button_text = !empty($workflow['button_text']) ? $workflow['button_text'] : 'Trigger Workflow';

    return '<button class="n8n-trigger-button" data-workflow-id="' . esc_attr($atts['id']) . '">' . esc_
...
function n8n_trigger_shortcode($atts) {
    ...
    return '<button class="n8n-trigger-button" data-workflow-id="' . esc_attr($atts['id']) . '">' . esc_html($button_text) . '</button>';
    }
